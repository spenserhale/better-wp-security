...
class ITSEC_Lib_Fingerprinting {
	...
	public static function applies_to_user( $user = false ) {
		...
		$had_filter = pbhs_remove_filter( 'user_has_cap', array( 'ITSEC_Fingerprinting', 'restrict_capabilities' ), 10 );
		...
		if ( $had_filter ) {
			pbhs_add_filter( 'user_has_cap', array( 'ITSEC_Fingerprinting', 'restrict_capabilities' ), 10, 4 );
		}
