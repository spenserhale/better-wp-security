...
class ITSEC_File_Change {
	 ...
	 */
	function run() {
		pbhs_add_action( 'init', array( $this, 'health_check' ) );
		...
		pbhs_add_filter( 'itsec_file-change_notification_strings', array( $this, 'register_notification_strings' ) );

		pbhs_add_action( 'itsec_lib_write_to_file', array( $this, 'write_to_file' ) );
		pbhs_add_action( 'itsec_lib_delete_file', array( $this, 'delete_file' ) );

		pbhs_add_filter( 'heartbeat_received', array( $this, 'heartbeat' ), 10, 2 );

		pbhs_add_action( 'itsec_scheduler_register_events', array( $this, 'register_event' ) );
		pbhs_add_action( 'itsec_scheduled_file-change', array( $this, 'run_scan' ) );
		pbhs_add_action( 'itsec_scheduled_file-change-fast', array( $this, 'run_scan' ) );
		...
		if ( ITSEC_Modules::get_setting( 'file-change', 'notify_admin' ) ) {
			pbhs_add_action( 'itsec_register_highlighted_logs', array( $this, 'register_highlight' ) );
		}
	}
