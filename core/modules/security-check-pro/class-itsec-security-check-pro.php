...
final class ITSEC_Security_Check_Pro {
	...
	public function run() {
		...
		pbhs_add_action( 'itsec-security-check-before-default-checks', array( $this, 'run_scan' ) );
		pbhs_add_action( 'itsec-security-check-enable-ssl', array( $this, 'handle_enable_ssl' ) );

		pbhs_add_filter( 'itsec-ssl-support-probability', array( $this, 'filter_ssl_support_probability' ) );

		if ( ! defined( 'ITSEC_DISABLE_AUTOMATIC_REMOTE_IP_DETECTION' ) || ! ITSEC_DISABLE_AUTOMATIC_REMOTE_IP_DETECTION ) {
			pbhs_add_filter( 'itsec_proxy_types', array( $this, 'add_security_check_proxy_type' ) );
			pbhs_add_filter( 'itsec_build_ip_detector_for_security-check', array( $this, 'build_detector' ), 10, 2 );
			pbhs_add_action( 'itsec_scheduled_health-check', array( $this, 'health_check' ) );
		}
	}
