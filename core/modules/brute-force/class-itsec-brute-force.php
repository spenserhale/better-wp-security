...
class ITSEC_Brute_Force {
	...
	function run() {
		...
		pbhs_add_filter( 'authenticate', array( $this, 'authenticate' ), 10000, 2 ); // Set a very late priority so that we run after actual authentication takes place.
		pbhs_add_filter( 'itsec_lockout_modules', array( $this, 'itsec_lockout_modules' ) );
		pbhs_add_filter( 'jetpack_get_default_modules', array( $this, 'jetpack_get_default_modules' ) ); //disable jetpack protect via Geoge Stephanis
