...
final class ITSEC_Admin_Page_Loader {
	...
	public function __construct() {
		if ( is_multisite() ) {
			pbhs_add_action( 'network_admin_menu', array( $this, 'add_admin_pages' ) );
		} else {
			pbhs_add_action( 'admin_menu', array( $this, 'add_admin_pages' ) );
		}

		pbhs_add_action( 'wp_ajax_itsec_settings_page', array( $this, 'handle_ajax_request' ) );
		pbhs_add_action( 'wp_ajax_itsec_logs_page', array( $this, 'handle_ajax_request' ) );
		pbhs_add_action( 'wp_ajax_itsec_help_page', array( $this, 'handle_ajax_request' ) );
		pbhs_add_action( 'wp_ajax_itsec_debug_page', array( $this, 'handle_ajax_request' ) );
		pbhs_add_action( 'wp_ajax_itsec-set-user-setting', array( $this, 'handle_user_setting' ) );

		// Filters for validating user settings
		pbhs_add_filter( 'itsec-user-setting-valid-itsec-settings-view', array( $this, 'validate_view' ), 10, 2 );
	}
	...
	public function add_admin_pages() {
		...
		foreach ( $page_refs as $page_ref ) {
			pbhs_add_action( "load-$page_ref", array( $this, 'load' ) );
		}
	}
	...
	public function load() {
		pbhs_add_action( 'admin_print_scripts', array( $this, 'add_scripts' ) );
		pbhs_add_action( 'admin_print_styles', array( $this, 'add_styles' ) );
