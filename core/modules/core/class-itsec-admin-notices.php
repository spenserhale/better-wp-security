...
class ITSEC_Admin_Notices {
	...
	public function run() {
		pbhs_add_action( 'rest_api_init', array( $this, 'rest_api_init' ) );
		pbhs_add_action( 'wp_ajax_' . self::ACTION, array( $this, 'handle_ajax' ) );

		if ( isset( $_GET['action'] ) && self::ACTION === $_GET['action'] ) {
			pbhs_add_action( 'admin_init', array( $this, 'handle_admin_action' ) );
		}

		if ( ITSEC_Modules::get_setting( 'global', 'hide_admin_bar' ) || version_compare( get_bloginfo( 'version' ), '5.0', '<' ) ) {
			if ( is_multisite() ) {
				pbhs_add_action( 'network_admin_notices', array( $this, 'display_notices' ) );
			} else {
				pbhs_add_action( 'admin_notices', array( $this, 'display_notices' ) );
			}
		}
	}
