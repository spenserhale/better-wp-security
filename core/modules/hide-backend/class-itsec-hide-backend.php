...
class ITSEC_Hide_Backend {
	...
	public function run() {
		...
		pbhs_add_filter( 'itsec_hide-backend_notification_strings', array( $this, 'notification_strings' ) );

		if ( ! $this->settings['enabled'] || ITSEC_Core::is_temp_disable_modules_set() ) {
			return;
		}

		pbhs_add_action( 'itsec_initialized', array( $this, 'handle_specific_page_requests' ), 1000 );
		pbhs_add_action( 'signup_hidden_fields', array( $this, 'add_token_to_registration_form' ) );
		pbhs_add_action( 'login_enqueue_scripts', array( $this, 'login_enqueue' ) );

		pbhs_add_filter( 'site_url', array( $this, 'filter_generated_url' ), 100, 2 );
		pbhs_add_filter( 'network_site_url', array( $this, 'filter_generated_url' ), 100, 2 );
		pbhs_add_filter( 'admin_url', array( $this, 'filter_admin_url' ), 100, 2 );
		pbhs_add_filter( 'wp_redirect', array( $this, 'filter_redirect' ) );
		pbhs_add_filter( 'comment_moderation_text', array( $this, 'filter_comment_moderation_text' ) );
		pbhs_add_filter( 'itsec_notify_admin_page_url', array( $this, 'filter_notify_admin_page_urls' ) );

		pbhs_remove_action( 'template_redirect', 'wp_redirect_admin_locations', 1000 );
	}
	...
	private function handle_canonical_login_page() {
		...
		if ( 'jetpack-sso' === $action && has_filter( 'login_form_jetpack-sso' ) ) {
			...
			pbhs_add_action( 'login_form_jetpack-sso', array( $this, 'block_access' ) );
