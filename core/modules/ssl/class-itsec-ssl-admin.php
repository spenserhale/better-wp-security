...
class ITSEC_SSL_Admin {
	function run() {
		...
		if ( 'advanced' === $settings['require_ssl'] && 1 === $settings['frontend'] ) {
			pbhs_add_action( 'init', array( $this, 'register_meta' ) );
			pbhs_add_action( 'enqueue_block_editor_assets', array( $this, 'enqueue_block_editor' ) );
			pbhs_add_action( 'post_submitbox_misc_actions', array( $this, 'ssl_enable_per_content' ) );
			pbhs_add_action( 'save_post', array( $this, 'save_post' ) );
		}
	}
