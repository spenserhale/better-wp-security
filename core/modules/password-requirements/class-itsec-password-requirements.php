...
class ITSEC_Password_Requirements {
	...
	public function run() {
		...
		pbhs_add_action( 'user_profile_update_errors', array( $this, 'forward_profile_pass_update' ), 0, 3 );
		pbhs_add_action( 'validate_password_reset', array( $this, 'forward_reset_pass' ), 10, 2 );

		pbhs_add_action( 'profile_update', array( $this, 'handle_update_user' ), 10, 2 );
		pbhs_add_action( 'password_reset', array( $this, 'handle_password_reset' ), 10, 2 );
		pbhs_add_filter( 'wp_authenticate_user', array( $this, 'check_password_on_login' ), 999, 2 );

		pbhs_add_action( 'add_user_role', array( $this, 'handle_role_change' ) );
		pbhs_add_action( 'set_user_role', array( $this, 'handle_role_change' ) );
		pbhs_add_action( 'remove_user_role', array( $this, 'handle_role_change' ) );

		pbhs_add_action( 'itsec_validate_password', array( $this, 'validate_password' ), 10, 4 );

		// This needs to run before the interstitial runs.
		pbhs_add_action( 'wp_login', array( $this, 'flag_check' ), - 2000, 2 );

		pbhs_add_action( 'itsec_login_interstitial_init', array( $this, 'register_interstitial' ) );
	}
