...
final class ITSEC_Notification_Center {
	...
	public function send( $notification, $mail ) {
		...
		pbhs_add_action( 'wp_mail_failed', array( $this, 'capture_mail_fail' ) );
		...
		pbhs_remove_action( 'wp_mail_failed', array( $this, 'capture_mail_fail' ) );
	 ...
	 */
	public function run() {
		pbhs_add_action( 'itsec_change_admin_user_id', array( $this, 'update_notification_user_id_on_admin_change' ) );
		pbhs_add_action( 'itsec_module_settings_after_title', array( $this, 'display_notification_center_link_for_module' ) );
		pbhs_add_action( 'itsec_register_highlighted_logs', array( $this, 'register_highlighted_log' ) );
	...
	private function setup_scheduling() {
		if ( $this->use_cron ) {
			...
			pbhs_add_action( self::CRON_ACTION, array( $this, 'check_notification_schedule_accurate' ) );
		} else {
			pbhs_add_action( 'init', array( $this, 'check_notification_schedule_fast' ), 20 );
		}
	}
