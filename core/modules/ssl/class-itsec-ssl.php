...
class ITSEC_SSL {
	...
	public function init() {
		...
		pbhs_add_action( 'template_redirect', array( $this, 'do_conditional_ssl_redirect' ), 0 );
		$settings = ITSEC_Modules::get_settings( 'ssl' );

		if ( 'enabled' === $settings['require_ssl'] ) {
			pbhs_add_filter( 'option_siteurl', array( $this, 'get_https_url' ), 5 );
			pbhs_add_filter( 'option_home', array( $this, 'get_https_url' ), 5 );
		}
		...
		if ( is_ssl() ) {
			...
			pbhs_add_filter( 'the_content', array( $this, 'replace_content_urls' ) );
			pbhs_add_filter( 'script_loader_src', array( $this, 'script_loader_src' ) );
			pbhs_add_filter( 'style_loader_src', array( $this, 'style_loader_src' ) );
			pbhs_add_filter( 'upload_dir', array( $this, 'upload_dir' ) );
