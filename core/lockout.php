...
final class ITSEC_Lockout {
	...
	public function run() {
		pbhs_add_action( 'itsec_scheduler_register_events', array( $this, 'register_events' ) );
		pbhs_add_action( 'itsec_scheduled_purge-lockouts', array( $this, 'purge_lockouts' ) );

		//Check for host lockouts
		pbhs_add_action( 'init', array( $this, 'check_for_host_lockouts' ) );

		// Ensure that locked out users are prevented from checking logins.
		pbhs_add_filter( 'authenticate', array( $this, 'check_authenticate_lockout' ), 30 );
		...
		if ( ! defined( 'ITSEC_DISABLE_TEMP_WHITELIST' ) || ! ITSEC_DISABLE_TEMP_WHITELIST ) {
			pbhs_add_action( 'init', array( $this, 'update_temp_whitelist' ), 0 );
		}

		//Register all plugin modules
		pbhs_add_action( 'plugins_loaded', array( $this, 'register_modules' ) );

		pbhs_add_action( 'ithemes_sync_register_verbs', array( $this, 'register_sync_verbs' ) );
		pbhs_add_filter( 'itsec-filter-itsec-get-everything-verbs', array( $this, 'register_sync_get_everything_verbs' ) );

		pbhs_add_action( 'itsec-settings-page-init', array( $this, 'init_settings_page' ) );
		pbhs_add_action( 'itsec-logs-page-init', array( $this, 'init_settings_page' ) );

		pbhs_add_filter( 'itsec_notifications', array( $this, 'register_notification' ) );
		pbhs_add_filter( 'itsec_lockout_notification_strings', array( $this, 'notification_strings' ) );

		pbhs_add_filter( 'itsec_logs_prepare_lockout_entry_for_list_display', array( $this, 'filter_entry_for_list_display' ), 10, 3 );
	}
	...
	public function execute_lock( $context = array() ) {
		...
		pbhs_add_filter( 'wp_die_handler', function () {
