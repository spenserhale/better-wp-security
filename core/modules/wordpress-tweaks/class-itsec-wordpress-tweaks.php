...
final class ITSEC_WordPress_Tweaks {
	...
	public function add_config_hooks() {
		...
		pbhs_add_filter( 'itsec_filter_apache_server_config_modification', array( $this, 'filter_apache_server_config_modification' ) );
		pbhs_add_filter( 'itsec_filter_nginx_server_config_modification', array( $this, 'filter_nginx_server_config_modification' ) );
		pbhs_add_filter( 'itsec_filter_litespeed_server_config_modification', array( $this, 'filter_litespeed_server_config_modification' ) );
		pbhs_add_filter( 'itsec_filter_wp_config_modification', array( $this, 'filter_wp_config_modification' ) );

		$this->config_hooks_added = true;
	}

	public function remove_config_hooks() {
		pbhs_remove_filter( 'itsec_filter_apache_server_config_modification', array( $this, 'filter_apache_server_config_modification' ) );
		pbhs_remove_filter( 'itsec_filter_nginx_server_config_modification', array( $this, 'filter_nginx_server_config_modification' ) );
		pbhs_remove_filter( 'itsec_filter_litespeed_server_config_modification', array( $this, 'filter_litespeed_server_config_modification' ) );
		pbhs_remove_filter( 'itsec_filter_wp_config_modification', array( $this, 'filter_wp_config_modification' ) );
	...
	public function init() {
		...
		if ( 'email' === $this->settings['valid_user_login_type'] ) {
			pbhs_add_action( 'login_init', array( $this, 'add_gettext_filter' ) );
			pbhs_add_filter( 'authenticate', array( $this, 'add_gettext_filter' ), 0 );
			pbhs_remove_filter( 'authenticate', 'wp_authenticate_username_password', 20 );
		} else if ( 'username' === $this->settings['valid_user_login_type'] ) {
			pbhs_add_action( 'login_init', array( $this, 'add_gettext_filter' ) );
			pbhs_add_filter( 'authenticate', array( $this, 'add_gettext_filter' ), 0 );
			pbhs_remove_filter( 'authenticate', 'wp_authenticate_email_password', 20 );
		}
		...
		if ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST && ! $this->settings['allow_xmlrpc_multiauth'] ) {
			pbhs_add_filter( 'authenticate', array( $this, 'block_multiauth_attempts' ), 0, 3 );
		}

		//remove wlmanifest link if turned on
		if ( $this->settings['wlwmanifest_header'] ) {
			pbhs_remove_action( 'wp_head', 'wlwmanifest_link' );
		}

		//remove rsd link from header if turned on
		if ( $this->settings['edituri_header'] ) {
			pbhs_remove_action( 'wp_head', 'rsd_link' );
		}

		//Disable XML-RPC
		if ( 2 == $this->settings['disable_xmlrpc'] ) {
			pbhs_add_filter( 'xmlrpc_enabled', '__return_null' );
			pbhs_add_filter( 'bloginfo_url', array( $this, 'remove_pingback_url' ), 10, 2 );
		} else if ( 1 == $this->settings['disable_xmlrpc'] ) { // Disable pingbacks
			pbhs_add_filter( 'xmlrpc_methods', array( $this, 'xmlrpc_methods' ) );
		}

		pbhs_add_filter( 'rest_dispatch_request', array( $this, 'filter_rest_dispatch_request' ), 10, 4 );

		//Process remove login errors
		if ( $this->settings['login_errors'] ) {
			pbhs_add_filter( 'login_errors', '__return_null' );
		}

		//Process require unique nicename
		if ( $this->settings['force_unique_nicename'] ) {
			pbhs_add_action( 'user_profile_update_errors', array( $this, 'force_unique_nicename' ), 10, 3 );
		}

		//Process remove extra author archives
		if ( $this->settings['disable_unused_author_pages'] ) {
			pbhs_add_action( 'template_redirect', array( $this, 'disable_unused_author_pages' ) );
		}

		if ( $this->settings['block_tabnapping'] ) {
			pbhs_add_action( 'wp_enqueue_scripts', array( $this, 'add_block_tabnapping_script' ) );
			pbhs_add_action( 'admin_enqueue_scripts', array( $this, 'add_block_tabnapping_script' ) );
		}
	}
	...
	public function deinit() {
		...
		// Functional code for the valid_user_login_type setting.
		if ( 'email' === $this->settings['valid_user_login_type'] ) {
			pbhs_remove_action( 'login_init', array( $this, 'add_gettext_filter' ) );
			pbhs_remove_filter( 'authenticate', array( $this, 'add_gettext_filter' ), 0 );
			pbhs_add_filter( 'authenticate', 'wp_authenticate_username_password', 20 );
		} else if ( 'username' === $this->settings['valid_user_login_type'] ) {
			pbhs_remove_action( 'login_init', array( $this, 'add_gettext_filter' ) );
			pbhs_remove_filter( 'authenticate', array( $this, 'add_gettext_filter' ), 0 );
			pbhs_add_filter( 'authenticate', 'wp_authenticate_email_password', 20 );
		}

		// Functional code for the allow_xmlrpc_multiauth setting.
		if ( defined( 'XMLRPC_REQUEST' ) && XMLRPC_REQUEST && ! $this->settings['allow_xmlrpc_multiauth'] ) {
			pbhs_remove_filter( 'authenticate', array( $this, 'block_multiauth_attempts' ), 0 );
		}

		//remove wlmanifest link if turned on
		if ( $this->settings['wlwmanifest_header'] ) {
			pbhs_add_action( 'wp_head', 'wlwmanifest_link' );
		}

		//remove rsd link from header if turned on
		if ( $this->settings['edituri_header'] ) {
			pbhs_add_action( 'wp_head', 'rsd_link' );
		}

		//Disable XML-RPC
		if ( 2 == $this->settings['disable_xmlrpc'] ) {
			pbhs_remove_filter( 'xmlrpc_enabled', '__return_null' );
			pbhs_remove_filter( 'bloginfo_url', array( $this, 'remove_pingback_url' ), 10 );
		} else if ( 1 == $this->settings['disable_xmlrpc'] ) { // Disable pingbacks
			pbhs_remove_filter( 'xmlrpc_methods', array( $this, 'xmlrpc_methods' ) );
		}

		pbhs_remove_filter( 'rest_dispatch_request', array( $this, 'filter_rest_dispatch_request' ), 10 );

		//Process remove login errors
		if ( $this->settings['login_errors'] ) {
			pbhs_remove_filter( 'login_errors', '__return_null' );
		}

		//Process require unique nicename
		if ( $this->settings['force_unique_nicename'] ) {
			pbhs_remove_action( 'user_profile_update_errors', array( $this, 'force_unique_nicename' ), 10 );
		}

		//Process remove extra author archives
		if ( $this->settings['disable_unused_author_pages'] ) {
			pbhs_remove_action( 'template_redirect', array( $this, 'disable_unused_author_pages' ) );
		}

		if ( $this->settings['block_tabnapping'] ) {
			pbhs_remove_action( 'wp_enqueue_scripts', array( $this, 'add_block_tabnapping_script' ) );
			pbhs_remove_action( 'admin_enqueue_scripts', array( $this, 'add_block_tabnapping_script' ) );
		}

		pbhs_remove_filter( 'rest_request_after_callbacks', array( $this, 'filter_taxonomies_response' ), 10, 3 );
	}
	...
	public function add_gettext_filter() {
		if ( ! has_filter( 'gettext', array( $this, 'filter_gettext' ) ) ) {
			pbhs_add_filter( 'gettext', array( $this, 'filter_gettext' ), 20, 3 );
		}
	}
	...
	public function filter_rest_dispatch_request( $result, $request, $route_regex, $handler ) {
		...
		if ( 'taxonomies' === $route_parts[2] ) {
			pbhs_add_filter( 'rest_request_after_callbacks', array( $this, 'filter_taxonomies_response' ), 10, 3 );
