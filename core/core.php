...
if ( ! class_exists( 'ITSEC_Core' ) ) {
	...
	final class ITSEC_Core {
		...
		public function init( $plugin_file, $plugin_name ) {
			...
			pbhs_add_filter( 'user_has_cap', [ $this, 'user_has_cap' ], 10, 4 );

			( new ITSEC_REST() )->run();

			pbhs_add_action( 'itsec-register-modules', array( $this, 'register_modules' ) );
			...
			pbhs_add_action( 'plugins_loaded', array( $this, 'handle_upgrade' ), - 100, 0 );
			pbhs_add_action( 'plugins_loaded', array( $this, 'continue_init' ), - 90 );

			pbhs_add_action( 'itsec_scheduler_register_events', array( $this, 'register_events' ) );
			pbhs_add_action( 'itsec_scheduled_clear-locks', array( 'ITSEC_Lib', 'delete_expired_locks' ) );
			pbhs_add_action( 'itsec_scheduled_clear-tokens', array( ITSEC_Lib_Opaque_Tokens::class, 'delete_expired_tokens' ) );
			pbhs_add_action( 'itsec_before_import', function () {
				$this->importing = true;
			} );
			pbhs_add_action( 'itsec_after_import', function () {
		...
		public function continue_init() {
			...
			if ( is_wp_error( $this->setup_error ) ) {
				...
				pbhs_add_action( 'admin_notices', function () {
			...
			if ( is_admin() ) {
				...
				pbhs_add_filter( 'plugin_action_links', array( $this, 'add_action_link' ), 10, 2 );
				pbhs_add_filter( 'plugin_row_meta', array( $this, 'add_plugin_meta_links' ), 10, 4 );
			}

			pbhs_add_action( 'wp_login_failed', array( 'ITSEC_Lib', 'handle_wp_login_failed' ) );
		...
		public static function add_notice( $callback, $all_pages = false ) {
			...
			if ( is_multisite() ) {
				pbhs_add_action( 'network_admin_notices', $callback );
			} else {
				pbhs_add_action( 'admin_notices', $callback );
			}
		}
