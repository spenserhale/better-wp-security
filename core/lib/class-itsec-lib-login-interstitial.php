...
class ITSEC_Lib_Login_Interstitial {
	...
	public function run() {
		...
		pbhs_add_action( 'login_enqueue_scripts', array( $this, 'enqueue' ) );
		pbhs_add_action( 'wp_login', array( $this, 'wp_login' ), - 1000, 2 );
		pbhs_add_action( 'wp_login_errors', array( $this, 'handle_token_expired' ) );
		pbhs_add_action( 'login_init', array( $this, 'force_interstitial' ) );
		pbhs_add_action( 'login_form', array( $this, 'ferry_after_login' ) );
		pbhs_add_filter( 'auth_cookie', array( $this, 'capture_session_token' ), 10, 5 );

		pbhs_add_action( 'wp_ajax_' . self::AJAX, array( $this, 'ajax_handler' ) );
		pbhs_add_action( 'wp_ajax_nopriv_' . self::AJAX, array( $this, 'ajax_handler' ) );

		foreach ( $this->registered as $id => $interstitial ) {
			if ( $interstitial->has_async_action() ) {
				pbhs_add_action( "login_form_itsec-{$id}", array( $this, 'async_action' ), 8 );
			}

			pbhs_add_action( "login_form_itsec-{$id}", array( $this, 'submit' ), 9 );
			pbhs_add_action( "login_form_itsec-{$id}", array( $this, 'display' ) );
		}
	}
	...
	protected function login_html( ITSEC_Login_Interstitial_Session $session ) {
		...
		pbhs_add_filter( 'jetpack_sso_allowed_actions', '__return_empty_array' );
	...
	private function handle_interstitials_completed( ITSEC_Login_Interstitial_Session $session, array $args ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			pbhs_remove_action( 'wp_login', array( $this, 'wp_login' ), - 1000 );
